...
* ```
*/
function getSunrise$1(latitude, longitude, date = /* @__PURE__ */ new Date(), options) {
...
* ```
*/
function getSunset$1(latitude, longitude, date = /* @__PURE__ */ new Date(), options) {
	const spa = initSpaFromDate(date, latitude, longitude, options);
...
* ```
*/
function getSolarNoon$1(latitude, longitude, date = /* @__PURE__ */ new Date(), options) {
	const spa = initSpaFromDate(date, latitude, longitude, options);
...
* ```
*/
function getTwilight$1(latitude, longitude, date = /* @__PURE__ */ new Date(), options) {
	const spa = initSpaFromDate(date, latitude, longitude, options);
	if (spaCalculate(spa) !== 0 || !isValidSunTime(spa.suntransit)) return null;
...
* ```
*/
function getSunTimes$1(latitude, longitude, date = /* @__PURE__ */ new Date(), options) {
	const spa = initSpaFromDate(date, latitude, longitude, options);
...
function getNativeTemporal() {
	const temporal = globalThis.Temporal;
	if (!temporal) throw new Error("Temporal is not available in this runtime. Use Node.js 26+ or a runtime with native Temporal support.");
	return temporal;
}
...
function isInstant(value) {
	return typeof value.epochMilliseconds === "number" && typeof value.toZonedDateTimeISO === "function";
}
function isZonedDateTime(value) {
	return typeof value.epochMilliseconds === "number" && typeof value.toInstant === "function" && typeof value.toPlainDate === "function";
}
function isPlainDateTime(value) {
	return typeof value.toZonedDateTime === "function" && typeof value.hour === "number";
}
function isPlainDate(value) {
	return typeof value.toPlainDateTime === "function";
}
function getTimezoneId(date, options) {
...
function localNoonDateFromCalendar(year, month, day, options) {
	return localDateTimeFromCalendar(year, month, day, 12, 0, 0, 0, options);
}
function localDateTimeFromCalendar(year, month, day, hour, minute, second, millisecond, options) {
	const timezone = getTimezone(options);
	if (timezone !== void 0) return new Date(Date.UTC(year, month - 1, day, hour - timezone, minute, second, millisecond));
	return new Date(year, month - 1, day, hour, minute, second, millisecond);
}
function toDate(date, options) {
	if (isDate(date)) return date;
	if (isInstant(date) || isZonedDateTime(date)) return new Date(date.epochMilliseconds);
	const timezoneId = getTimezoneId(date, options);
	if (timezoneId) {
		if (isPlainDateTime(date)) return new Date(date.toZonedDateTime(timezoneId).toInstant().epochMilliseconds);
		if (isPlainDate(date)) return new Date(date.toPlainDateTime({
			hour: 12,
			minute: 0,
			second: 0,
			millisecond: 0
		}).toZonedDateTime(timezoneId).toInstant().epochMilliseconds);
	}
	if (isPlainDateTime(date)) return localDateTimeFromCalendar(date.year, date.month, date.day, date.hour, date.minute, date.second, date.millisecond, options);
	return localNoonDateFromCalendar(date.year, date.month, date.day, options);
}
function mergeOptions(date, options) {
	const timezoneId = getTimezoneId(date, options);
	if (!timezoneId || options?.timezoneId) return options;
	return {
		...options,
		timezoneId
	};
...
function toTemporalTwilight(twilight) {
...
function getSunrise(latitude, longitude, date = /* @__PURE__ */ new Date(), options) {
	return toInstant(getSunrise$1(latitude, longitude, toDate(date, options), mergeOptions(date, options)));
}
function getSunset(latitude, longitude, date = /* @__PURE__ */ new Date(), options) {
	return toInstant(getSunset$1(latitude, longitude, toDate(date, options), mergeOptions(date, options)));
}
function getSolarNoon(latitude, longitude, date = /* @__PURE__ */ new Date(), options) {
	return toInstant(getSolarNoon$1(latitude, longitude, toDate(date, options), mergeOptions(date, options)));
}
function getTwilight(latitude, longitude, date = /* @__PURE__ */ new Date(), options) {
	return toTemporalTwilight(getTwilight$1(latitude, longitude, toDate(date, options), mergeOptions(date, options)));
}
function getSunTimes(latitude, longitude, date = /* @__PURE__ */ new Date(), options) {
	const dateOptions = mergeOptions(date, options);
	const times = getSunTimes$1(latitude, longitude, toDate(date, options), dateOptions);
	return {
...
//#endregion
export { getSolarNoon, getSunTimes, getSunrise, getSunset, getTwilight };
